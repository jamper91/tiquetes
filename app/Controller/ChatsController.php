...
EOD;
	...
	$sql = "update chat set recd = 1 where chat.to = '".mysql_real_escape_string($this->Session->read('User.type_user_id'))."' and recd = 0";
	//$query = mysql_query($sql);
	//$query=$this->Event->query($sql);
...
?>
{
		"username": "<?php echo $this->Session->read('nameUser');?>",
		"items": [
			<?php echo $items;?>
        ]
}
...
function sendChat() {
	...
	$this->loadModel('Event');
	$from=$this->Session->read('nameUser');
	$to = $_POST['to'];
