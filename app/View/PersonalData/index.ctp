<div class="personalData index">
	<h2><?php echo __('Personal Data'); ?></h2>
	<table cellpadding="0" cellspacing="0">
	<thead>
	<tr>
			<th><?php echo $this->Paginator->sort('id'); ?></th>
			<th><?php echo $this->Paginator->sort('descripcion'); ?></th>
			<th><?php echo $this->Paginator->sort('id_padre'); ?></th>
			<th><?php echo $this->Paginator->sort('tipo'); ?></th>
			<th><?php echo $this->Paginator->sort('obligatorio'); ?></th>
			<th class="actions"><?php echo __('Actions'); ?></th>
	</tr>
	</thead>
	<tbody>
	<?php foreach ($personalData as $personalDatum): ?>
	<tr>
		<td><?php echo h($personalDatum['PersonalDatum']['id']); ?>&nbsp;</td>
		<td><?php echo h($personalDatum['PersonalDatum']['descripcion']); ?>&nbsp;</td>
		<td><?php echo h($personalDatum['PersonalDatum']['id_padre']); ?>&nbsp;</td>
		<td><?php echo h($personalDatum['PersonalDatum']['tipo']); ?>&nbsp;</td>
		<td><?php echo h($personalDatum['PersonalDatum']['obligatorio']); ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $personalDatum['PersonalDatum']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $personalDatum['PersonalDatum']['id']), array(), __('Are you sure you want to delete # %s?', $personalDatum['PersonalDatum']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</tbody>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>
	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>